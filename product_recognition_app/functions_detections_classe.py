...
def run_detector_fasterRCNN_first_classe(detector, path,classe_name):
    a = {}
    tailles={}
    i=0
    img = load_img(path)
    ...
    while(i<= (len(result["detection_scores"])-1)):
        ...
        i=i+1
    remove_classes_filter(a,classe_name,classes,result_boxes,result_scores,result_taille)
...
def run_detector_retinaNet_second_classe(path,model,idUser,classe_name,classes_Faster,scores_Faster,boxes_Faster,heights,product_dict,tailles,result_taille):
    ...
    result_scores=scores_Faster+scores
    result_boxes=boxes_Faster+boxes
    ...
    add_shelf(heights,products,result_scores,result_boxes,1.0)
    a["Etagere"]=len(heights)-1   
    X = np.array(result_boxes)
    list_result["detection_class_entities"]=products
    list_result["detection_scores"]=result_scores
    list_result["detection_boxes"]= X
    print("le nombre total des produits est : " ,len(classes+classes_Faster)-1)

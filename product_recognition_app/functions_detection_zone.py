...
def run_detector_fasterRCNN_first_zone(detector, path):
    a = {}
    tailles={}
    i=0
    img = load_img(path)
    ...
    while(i<= (len(result["detection_scores"])-1)):
        if((result["detection_class_entities"][i].decode() in accepted_classes_facter_RCNN )):
            ...
            if(score>0.996):
                ...
                taille=(result["detection_boxes"][i][2]-result["detection_boxes"][i][0])*(result["detection_boxes"][i][3]-result["detection_boxes"][i][1])
...
def run_detector_retinaNet_second_zone(path,model,idUser,nb_zone,classes_Faster,scores_Faster,boxes_Faster,heights,product_dict,tailles,result_taille):
    ...
    while(i<= len(detections.nmsed_classes[0][:num_detections])-1):
            ...
            if((predicted_class in accepted_classes_retinaNet_int )):
                ...
                im.save(path_im)
                size_im=im.size
                taille=(boxe[2]-boxe[0])*(boxe[3]-boxe[1])
                classe,score=predict_img(path_im,model_classification,class_names)
    ...
    for p in x:
        for z in x:
            ...
            zones.append(zone)
            nb+=1
    remove_classes_zone(products,product_dict,boxes,scores,result_taille,zones,nb_zone)
    ...
    list_result["detection_boxes"]= X
    print("le nombre total des produits est : " ,len(products)-len(heights))
